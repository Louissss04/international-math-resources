...
export function PlannerClient({ projects }: { projects: ProjectRecord[] }) {
  ...
  function exportJson() { download(JSON.stringify(state, null, 2), `mathpath-planner-${active.name.replaceAll(/\s+/g, "-")}.json`, "application/json;charset=utf-8"); }
  function exportCsv() {
    ...
    download(`\uFEFF${rows.map((row) => row.map(csvCell).join(",")).join("\r\n")}`, `mathpath-planner-${active.name.replaceAll(/\s+/g, "-")}.csv`, "text/csv;charset=utf-8");
  }
  function exportIcs() {
    ...
    download(["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//MathPath//Planner//ZH-EN", ...events, "END:VCALENDAR"].join("\r\n"), `mathpath-planner-${active.name.replaceAll(/\s+/g, "-")}.ics`, "text/calendar;charset=utf-8");
  }

  async function importJson(file: File) {
    try { setState(normalise(JSON.parse(await file.text()))); } catch { window.alert("Invalid MathPath planner backup."); }
    if (importRef.current) importRef.current.value = "";
  }

  if (!ready) return <p className="empty-state"><span className="lang-zh">正在读取本地规划数据…</span><span className="lang-en">Loading local planner data…</span></p>;
  ...
  return (
    <div className="planner-workspace">
      ...
      <section className="planner-add"><label><span className="lang-zh">加入项目</span><span className="lang-en">Add project</span><select value={projectToAdd} onChange={(event) => setProjectToAdd(event.target.value)}><option value="">选择 / Select</option>{available.map((project) => <option key={project.id} value={project.id}>{project.shortTitle} — {project.title.zh}</option>)}</select></label><button className="primary-button" type="button" onClick={addProject} disabled={!projectToAdd}><span className="lang-zh">加入</span><span className="lang-en">Add</span></button></section>
      ...
      {items.length ? <div className="planner-items">{items.map((item) => { const project = projects.find((entry) => entry.id === item.projectId); return <article key={item.projectId}><div><h2>{item.titleZh}<small>{item.titleEn}</small></h2>{project && <Link href={projectHref(project)}><span className="lang-zh">项目档案</span><span className="lang-en">Record</span></Link>}</div><label><span className="lang-zh">状态</span><span className="lang-en">Status</span><select value={item.status} onChange={(event) => patchItem(item.projectId, { status: event.target.value as ItemStatus })}><option value="researching">核验 / Researching</option><option value="preparing">准备 / Preparing</option><option value="submitted">已提交 / Submitted</option><option value="complete">完成 / Complete</option></select></label><label><span className="lang-zh">截止</span><span className="lang-en">Deadline</span><input type="date" value={item.deadline ?? ""} onChange={(event) => patchItem(item.projectId, { deadline: event.target.value })} /></label><label className="planner-note"><span className="lang-zh">备注</span><span className="lang-en">Note</span><textarea value={item.note ?? ""} onChange={(event) => patchItem(item.projectId, { note: event.target.value })} /></label><button className="remove-item" type="button" onClick={() => removeItem(item.projectId)}><span className="lang-zh">移除</span><span className="lang-en">Remove</span></button></article>; })}</div> : <p className="empty-state"><span className="lang-zh">此档案尚未加入项目。</span><span className="lang-en">No projects in this profile.</span></p>}
    </div>
  );
}
